<template>
  <div class="app-container">
    <el-row :gutter="15">
      <el-form ref="homeworkForm" :model="homeworkFormData" :rules="homeworkFormRules" size="medium"
        label-width="100px">
        <el-col :span="6">
          <el-form-item label="作业名" prop="name">
            <el-input v-model="homeworkFormData.name" placeholder="作业名" clearable :style="{ width: '100%' }">
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="限时" prop="limitTime">
            <el-input v-model="homeworkFormData.limitTime" placeholder="单位：分钟" clearable
              :style="{ width: '100%' }"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="满分" prop="fullGrade">
            <el-input v-model="homeworkFormData.fullGrade" placeholder="满分" clearable
              :style="{ width: '100%' }"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="课程" prop="course_id">
            <el-select v-model="homeworkFormData.course_id" placeholder="请选择课程" clearable :style="{ width: '100%' }">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['easystudy:question:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['easystudy:question:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['easystudy:question:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['easystudy:question:export']">导出</el-button>
      </el-col>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="题型" required>
          <el-radio-group v-model="form.type" @change="questionTypeChange">
            <el-radio-button v-for="entry in Object.entries(questionTypeDict)" :key="entry[1]"
              :label="entry[1]">{{ entry[0] }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="答案" prop="answer">
          <el-radio-group v-model="form.answer" v-if="form.type == questionTypeDict['单选题']">
            <el-radio-button v-for="entry in Object.entries(choiceQuestionAnswer)" :key="entry[0]"
              :label="entry[1]">{{ entry[0] }}</el-radio-button>
          </el-radio-group>
          <el-radio-group v-model="form.answer" v-if="form.type == questionTypeDict['判断题']">
            <el-radio-button v-for="entry in Object.entries(trueOrFalseQuestionAnswer)" :key="entry[0]"
              :label="entry[0]">{{ entry[1] }}</el-radio-button>
          </el-radio-group>
          <el-checkbox-group v-model="mutipleChoiceAnswer" v-if="form.type == questionTypeDict['多选题']">
            <el-checkbox-button v-for="entry in Object.entries(choiceQuestionAnswer)" :key="entry[0]"
              :label="entry[1]">{{ entry[0] }}</el-checkbox-button>
          </el-checkbox-group>
        ...
        <el-form-item label="难度" prop="difficulty" required>
          <el-radio-group v-model="form.difficulty">
            <el-radio-button v-for="entry in Object.entries(difficultyDict)" :key="entry[0]"
              :label="entry[1]">{{ entry[0] }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
...
import {
  questionTypeDict, difficultyDict, difficultyReverseDict
  , trueOrFalseQuestionAnswer, trueOrFalseReverseQuestionAnswer
  , choiceQuestionAnswer, choiceQuestionRverseAnswer
} from "@/utils/dictInMemory";
import { listAllCourse } from "@/api/easystudy/course";
import { regexConst } from '@/utils/regex';
export default {
  name: "addHomework",
  data() {
    return {
      ...
      homeworkFormRules: {
        ...
        limitTime: [{
          ...
          message: '限时是必须的',
          trigger: 'blur'
        }, { pattern: regexConst.positiveNumber, message: '必须为正数', trigger: 'blur' }],
        fullGrade: [{
          required: true,
          message: '满分是必须的',
          trigger: 'blur'
